...
export function CheckoutOne({ onContinueToPayment }: CheckoutOneProps) {
				...
				<form>
					<FieldGroup>
						...
						<FieldSet>
							...
							<RadioGroup
								defaultValue="home"
								onValueChange={(value) => setShippingMethod(value as "home" | "pickup")}
				...
				<ul className="space-y-4 max-h-[300px] overflow-y-auto">
					{cart?.items?.map((item) => (
						<li key={item.id} className="flex gap-3">
							...
							<p className="font-medium text-sm">R{(item.price * item.quantity).toLocaleString()}</p>
						</li>
					))}
					{!cart?.items?.length && (
